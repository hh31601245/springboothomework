...
public class R<T> implements Serializable {
    ...
    public static boolean isSuccess(@Nullable R<?> result)
    {
        return (Boolean)Optional.ofNullable(result).map((x)->{
            return ResultCode.SUCCESS.code==x.code;}).orElse(Boolean.FALSE);
    }
    ...
    public static final Key KEY=new SecretKeySpec("123456".getBytes(), SignatureAlgorithm.HS512.getJcaName());
